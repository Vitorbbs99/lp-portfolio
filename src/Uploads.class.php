...
class Uploads {
	...
	// Constantes
	const QUALIDADE = 90;		    // Qualidade da compressão
    ...
    if (exif_imagetype($pic) == IMAGETYPE_WEBP) {
			$image = imagecreatefromwebp($pic);
			imagewebp($image, $path, $qualidade);
			imagedestroy($image);
		}
    // Imagick
		...
		// GD
		else if (exif_imagetype($pic) == IMAGETYPE_JPEG) {
