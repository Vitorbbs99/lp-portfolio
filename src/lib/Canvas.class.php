...
class canvas {
  ...
  private function dados() {
    ...
    if (is_file($this->origem)) {
      ...
      if (!$this->eImagem()) {
        trigger_error('Erro: Arquivo ' . $this->origem . ' n�o é uma imagem!', E_USER_ERROR);
      }
      else {
        // pega dimensões
        $this->dimensoes();
  ...
  public function novaImagem($largura, $altura) {
    ...
    $cor_fundo = imagecolorallocatealpha($this->img, 0, 0, 0, 127);
  ...
  /**
   * Cria objeto de imagem para manipulaç�o no GD
   * @param
   * @return void
   */
  private function criaImagem() {
    switch ($this->formato) {
      ...
      break;
      case 6:
        $this->img = imagecreatefrombmp2($this->origem);
        $this->extensao = 'bmp';
      break;
      case 18:
   ...
   * @param String $tipo método para redimensionamento (padr�o [vazio], preenchimento ou crop)
   * @return Object instância atual do objeto, para métodos encadeados
   */
  ...
  private function preencheImagem() {
    ...
    */
    ...
    /**
     * Mant�m o fundo transparent
     */

    imagesavealpha($this->img_temp, true);
    $cor_fundo = imagecolorallocatealpha($this->img_temp, 0, 0, 0, 127);
    imagefill($this->img_temp, 0, 0, $cor_fundo);
  ...
  private function redimensionaPreenchimento() {
    ...
    // salva variáveis para centralizaç�o
  ...
  /**
   * Calcula a posiç�o do crop
   * Os índices 0 e 1 correspondem à posiç�o x e y do crop na imagem
   ...
   * @param Int $x posiç�o x do texto na imagem
   * @param Int $y posiç�o y do texto na imagem
   ...
   * @param Int/String $x posiç�o x da marca na imagem ou constante para marcaFixa()
   * @param Int/Sring $y posiç�o y da marca na imagem ou constante para marcaFixa()
   * @return Boolean true/false dependendo do resultado da operaç�o
   * @param Int $alfa valor para transparência (0-100)
   -> se utilizar alfa, a funç�o imagecopymerge n�o preserva
  ...
  public function marca($imagem, $x = 0, $y = 0, $alfa = 100) {
    ...
    if ($imagem) {
      ...
      switch (strtolower($pathinfo['extension'])) {
        ...
        break;
        case 'bmp':
          $marcadagua = imagecreatefrombmp2($imagem);
        break;
        default:
   ...
   * Implementaç�o original por Giolvani <inavloig@gmail.com>
   * @param String $imagem caminho da imagem de marca d'água
   * @param String $posicao posiç�o/orientaç�o fixa da marca d'água
   ...
   * Requer o GD compilado com a funç�o imagefilter()
   ...
   * @param $arg1, $arg2 e $arg3 - ver manual da funç�o imagefilter
   * @return Object instância atual do objeto, para métodos encadeados
   *
   */
